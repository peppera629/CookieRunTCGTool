...
public class CollectionSummaryDialog {
    ...
    private void initialize(boolean choiceMode) {
        ...
        for (int i = 0; i < 4; i++) {
            JLabel label = new JLabel("<html><img src=\"file:" + new File(AppPaths.dataDir().resolve("icons_rarity/24px/" + CardUtil.CardRarity.fromValue(6+i).getName() + ".png").toString()).getAbsolutePath() + "\"></html>", JLabel.CENTER);
            label.setAlignmentX(Component.CENTER_ALIGNMENT);
            label.setFont(MainUI.CRnormal);
            byRaritySecret.add(label, gbc_byRaritySecret);
